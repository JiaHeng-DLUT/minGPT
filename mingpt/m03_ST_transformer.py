...
from loss import cal_animal_loss, cal_frame_loss
from model3 import GPT, GPT1Config
from utils import set_random_seed


wandb.init(project="mouse", entity="jiaheng")
...
class TrainerConfig:
    ...
    train_dataset = {
        ...
        'meta_path': 'meta_info/mouse_meta_info_train_0.txt',
    ...
    val_dataset = {
        'data_path': '../../Mouse_Triplets/Notebooks/data/user_train.npy',
        'meta_path': 'meta_info/mouse_meta_info_val_0.txt',
        'num_frame': clip_frames,
        'total_frame': total_frames,
    }
    batch_size = 32
    ...
    # checkpoint settings
    ckpt_dir = f'./experiments/fly/m03_ST_transformer'
    # CUDA_VISIBLE_DEVICES=0 python m03_ST_transformer.py > experiments/fly/log/m03_ST_transformer.log
        ...
        def run_epoch(split):
            ...
            for it, data in pbar:
                ...
                if is_train:
                    b, t, c, _ = x.shape
                    losses = {
                        'l_animal_prev': cal_animal_loss(self.model.module.decoder_animal_prev(feat1[:, 1:] * mask[:, 1:, :, None]), x[:, :-1] * mask[:, :-1, :, None]),
                        'l_animal_next': cal_animal_loss(self.model.module.decoder_animal_next(feat1[:, :-1] * mask[:, :-1, :, None]), x[:, 1:] * mask[:, 1:, :, None]),
                        'l_frame_LR': cal_frame_loss(self.model.module.decoder_frame(feat2_LR[:, :-1]), x[:, 1:].view(b, t - 1, -1)),
                        'l_frame_RL': cal_frame_loss(self.model.module.decoder_frame(feat2_RL[:, 1:]), x[:, :-1].view(b, t - 1, -1)),
                    }
                    loss = 0
                    ...
                    feat = feat2_LR.view(-1, feat2_LR.shape[-1])
                    feats.append(feat)
...
if __name__ == '__main__':
    ...
    train_set = mouse_aug_dataset_3(config.train_dataset)
    val_set = mouse_aug_dataset_3(config.val_dataset)
