...
class TrainerConfig:
    ...
    # checkpoint settings
    ckpt_dir = f'./experiments/fly/15_same_13_F1'
    # CUDA_VISIBLE_DEVICES=0 python 15_same_13_F1.py > experiments/fly/log/15_same_13_F1.log
...
class Trainer:
    ...
    def train(self):
        ...
        def run_epoch(split):
            ...
            for it, data in pbar:
                ...
                with torch.set_grad_enabled(is_train):
                    feats, loss1, loss2, loss3, tp, fn, fp, tn = model(x, pos, y)
        ...
        if self.test_dataset is not None:
            best_f1 = run_epoch('test')
        for epoch in range(config.max_epochs):
            
            run_epoch('train')
            if self.test_dataset is not None:
                val_f1 = run_epoch('test')
