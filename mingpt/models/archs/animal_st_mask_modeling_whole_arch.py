...
class AnimalSTMaskModelingWhole(nn.Module):
    """
    ...
    def forward(self, tokens, masks, pos, flip=False, decode_animal=False, decode_frame=False):
        ...
        random_masks = torch.bernoulli(torch.ones(b, t, 1) * 0.5).to(masks)
        random_masks = random_masks.expand(-1, -1, c).reshape(b, t * c)
