...
class SGPT(nn.Module):
    ...
    def forward(self, tokens, mask):
        ...
        x, _ = self.blocks((x, mask.view(b * t, c)))
        x = self.ln_f(x)
        x = self.proj(x)
        return x.view(b, t, c, -1)

class TGPT(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        config.n_embd = config.n_embd * config.num_animals
        ...
        config.n_embd = int(config.n_embd / config.num_animals)

    def forward(self, feat1, pos):
        (b, t, _) = feat1.shape
        position_embeddings = []
        for i in range(b):
...
class GPT(nn.Module):
    ...
    def forward(self, tokens, pos, mask):
        ...
        feat2_LR, feat2_RL = self.tgpt(feat1.flatten(-2), pos)
        return feat1, feat2_LR, feat2_RL
