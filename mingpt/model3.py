...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.proj = nn.Sequential(nn.Linear(config.n_embd, config.n_embd, bias=False),
                                nn.BatchNorm1d(config.n_embd),
                                nn.ReLU(inplace=True), # first layer
                                nn.Linear(config.n_embd, config.n_embd, bias=False),
                                nn.BatchNorm1d(config.n_embd),
                                nn.ReLU(inplace=True), # second layer
                                nn.Linear(config.n_embd, config.output_dim, bias=False),
                                nn.BatchNorm1d(config.output_dim, affine=False)) # output layer
    ...
    def forward(self, tokens, pos, mask, y=None):
        ...
        x = self.ln_f(x)
        x = self.proj(x.view(b * t * c, -1)).view(b, t * c, -1)
        ...
        x2 = self.ln_f(x2)
        x2 = self.proj(x2.view(b * t * c, -1)).view(b, t * c, -1)
